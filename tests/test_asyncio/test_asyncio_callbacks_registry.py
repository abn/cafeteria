...
def test_callback_registry_simple(mocker: Any) -> None:
...
def test_callback_registry_multiple(mocker: Any) -> None:
    callback_one = mocker.Mock()
    callback_two = mocker.Mock()
...
def test_callback_registry_coro_without_loop(caplog: Any) -> None:
    async def hello():
        pass
    ...
    with pytest.warns(RuntimeWarning):
        ...
        assert f"skipping {hello!s}" in caplog.text


def test_callback_registry_callbacks(mocker: Any) -> None:
    callback_one = mocker.Mock()
    callback_two = mocker.Mock()
...
def test_callback_registry_exists(mocker: Any) -> None:
...
def test_callback_registry_callbacks_default(mocker: Any) -> None:
    callback = mocker.Mock()
    registry = CallbackRegistry()
...
def test_callback_registry_default(mocker: Any) -> None:
    callback = mocker.Mock()
    registry = CallbackRegistry()
    registry.register(None, callback)
...
def test_callback_registry_deregister_non_existing() -> None:
    try:
